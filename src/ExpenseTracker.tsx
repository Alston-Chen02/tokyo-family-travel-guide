...
const authLinkHost = "vpjfubcgdnoetawguogf.supabase.co";

function tokenFromSignInLink(value: string): { token_hash: string; type: "email" | "magiclink" } {
  let url: URL;
  try { url = new URL(value.trim()); }
  catch { throw new Error("請貼上信件中完整的登入連結，不是開啟後的網址"); }
  const type = url.searchParams.get("type");
  const token_hash = url.searchParams.get("token");
  if (url.protocol !== "https:" || url.hostname !== authLinkHost
    || url.pathname !== "/auth/v1/verify" || !token_hash
    || (type !== "email" && type !== "magiclink")) {
    throw new Error("這不是本旅程 Supabase 寄出的登入連結");
  }
...
export default function ExpenseTracker() {
  ...
  const sendLink = async (event: FormEvent) => {
    ...
    setBusy(true); setMessage(""); setSent(false); setLoginLink("");
  ...
  const completeSignIn = async (event: FormEvent) => {
    ...
    const enteredLink = loginLink;
    setLoginLink("");
    try {
      const token = tokenFromSignInLink(enteredLink);
      const { data, error } = await supabase.auth.verifyOtp(token);
      if (error) throw error;
      if (!data.session) throw new Error("尚未建立登入狀態");
      setSession(data.session);
      setSent(false);
    } catch (error) {
      setMessage(`登入失敗：${error instanceof Error ? error.message : "請重新寄送登入信"}`);
