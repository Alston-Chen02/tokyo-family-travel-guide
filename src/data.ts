...
export const RESERVATION_HUB = [
  {
    ...
    url: 'https://www.evaair.com/zh-tw/manage/manage-your-trip/',
    action: '管理行程',
  },
  {
    id: 'skyliner',
    ...
    meta: '09/19 成田機場 → 日暮里',
    status: '已購票',
    note: '京成電鐵官網已購買 09/19 成田機場 T1 → 日暮里單程票（2 大 1 小，¥5,770）。持購買完成信的 QR Code 或購買明細到京成櫃檯兌換並劃位；13:39 是目標班次，非已指定車次。',
  ...
  {
    id: 'aquarium',
    title: 'Maxell Aqua Park 品川',
    meta: '09/23 · 09:00 起可用（19:00 前入場）',
    status: '已購票',
    note: '成人票 2 張已付款（含 No-show Refund 共 NT$1,168），3 歲孩子免費；入場時直接出示已下載的 QR Code。水豚餵食目前因健康管理暫停，活動狀態仍須另查官方公告。',
    url: 'https://www.aqua-park.jp/aqua/event/31695/index.html',
    action: '水豚活動公告',
  },
];
...
export const MEDICAL_SEARCHES = [
  { label: '舞濱飯店附近兒科', query: '小児科 千葉県浦安市舞浜' },
  { label: '水道橋飯店附近兒科', query: '小児科 東京都文京区後楽 水道橋' },
  { label: '錦糸町飯店附近兒科', query: '小児科 東京都墨田区錦糸町' },
  { label: '東京急診／醫院', query: '救急外来 病院 東京都' },
];
...
export const LUGGAGE_ROUTE: LuggageRouteItem[] = [
  ...
  },
  {
    ...
    method: 'Airporter',
    date: '09/23',
    note: '09:00 前交由巨蛋飯店寄放 ➔ 20:00 於錦糸町樂天城市飯店領取 · 托運尺寸 2 件＋登機尺寸 1 件 · 掛牌填寫 09/23 這筆訂單號並依確認信上傳行李照片',
    cost: '已付 ¥6,710 / 3 件',
    status: 'paid',
  },
  {
...
export const DAYS: DayPlan[] = [
  {
    ...
    stops: [
      ...
      mkStop('d1s2', '13:39 - 14:18', '預計搭乘京成 Skyliner 前往日暮里', '成田機場 ➔ 日暮里 37 分鐘；再轉乘前往押上',
        '已直接向京成電鐵購買 09/19 成田機場第 1 航廈至日暮里的 Skyliner 單程票（2 大 1 小），但購買信未指定車次。入境、領行李與交付托運後，持信中的 QR Code 或購買明細於京成櫃檯兌換、選定班次與座位。13:39 為預定目標；若趕不上，就劃下一班。Skyliner 不直達押上，抵達日暮里後再轉乘前往晴空塔。',
        ['京成官網已購票', 'QR Code 兌換劃位', '13:39 為目標班次', '日暮里轉乘'],
        { type: 'train', label: '京成 Skyliner（現場劃位）', route: '成田機場 → 日暮里；再轉乘押上', duration: 'Skyliner 37 分＋轉乘約 20 分' },
        'Keisei Skyliner Narita Airport', 'tokyo', '京成電鐵電子票（已付款）', '已付 ¥5,770 / 2 大 1 小',
        'https://new-www.keisei.co.jp/keisei/tetudou/skyliner/jp/index.php'),
  ...
  },
  {
    ...
    stops: [
      ...
      mkStop('d2s5', '10:30 - 13:00', '夢幻樂園：提早午餐與主目標', '午餐 → 親子設施 → 美女與野獸',
        ...
        '東京ディズニーランド ファンタジーランド', 'urayasu', '午餐:園內主題餐', '預算依個人消費'),
      ...
      mkStop('d2s7', '15:30 - 18:30', '明日樂園 → 世界市集', '看體力選玩，17:00 前完成晚餐',
        ...
        '東京ディズニーランド トゥモローランド', 'urayasu', '晚餐:園內提前用餐', '預算依個人消費'),
  ...
  },
  {
    ...
    stops: [
      ...
      mkStop('d3s6', '18:00 - 19:30', 'Tokyo Dome City LaQua 商城逛街', 'LaQua 購物中心 (飯店步行 3 分鐘)',
        ...
        'LaQua ショップ&レストラン 東京ドームシティ', 'tokyo', '—', '購物預算 ¥3,000~¥8,000 / 人',
        'https://www.laqua.jp/'),
  ...
  },
  {
    ...
    stops: [
      ...
      mkStop('d4s6', '18:00 - 20:00', '晚餐：巨蛋商圈溫和系美饌', '東京巨蛋城 / 水道橋周邊',
        ...
        'LaQua ショップ&レストラン 東京ドームシティ', 'tokyo', '晚餐：清燉拉麵/手打烏龍麵/定食', '餐飲預算約 ¥1,500~¥2,500 / 人'),
    ],
  },
  {
    id: 'd5',
    ...
    dateLabel: '09/23',
    weekday: '三',
    theme: '海洋探索 • Aqua Park 水豚與海豚表演',
    cityLabel: '東京・水道橋 ➔ 品川 ➔ 錦糸町',
    cityKey: 'kinshicho',
    stops: [
      mkStop('d5s1', '08:30 - 09:00', '退房 & 行李託運 (巨蛋 ➔ 錦糸町)', '東京巨蛋飯店 ➔ 錦糸町樂天城市酒店',
        '務必於 09:00 前將 Airporter 的 3 件行李交由東京巨蛋飯店寄放（托運尺寸 2 件、登機尺寸 1 件）；依確認信將本筆訂單號填在行李掛牌並上傳行李照片。這是寄放截止時間，不是飯店退房期限。行李預定 20:00 於錦糸町樂天城市飯店領取；隨身帶親子用品與水族館所需物品。',
        ['09:00 前寄放 3 件', '掛牌與上傳行李照片', '20:00 飯店領取', '輕裝出發'],
        { type: 'walk', label: '館內', route: '飯店櫃檯', duration: '—' },
        'Tokyo Dome Hotel', 'tokyo', '—', '行李託運服務'),
      mkStop('d5s2', '09:00 - 09:40', '水道橋前往 Maxell Aqua Park 品川', 'JR 水道橋 ➔ 秋葉原 ➔ 品川；高輪口步行約 2 分',
        '由水道橋搭 JR 中央・總武線至秋葉原，轉京濱東北線南行至品川站；由高輪口出站，穿過 Wing 高輪往品川王子大飯店方向上坡，即抵達 Maxell Aqua Park。9/23 為國定假日，另留轉乘與排隊緩衝。',
        ['秋葉原轉乘', '品川站高輪口', '步行約 2-5 分', '假日預留緩衝'],
        { type: 'train', label: 'JR + 步行', route: '水道橋 → 秋葉原 → 品川 → Aqua Park', duration: '約 30-40 分' },
        'Maxell Aqua Park Shinagawa', 'tokyo', '—', 'IC 卡車資依當日票價'),
      mkStop('d5s3', '09:40 - 10:10', '入場・確認水豚與表演場次', 'Maxell Aqua Park 品川（品川王子大飯店內）',
        '成人票 2 張已購買，票券自 9/23 09:00 起可用、19:00 前有效；抵達後直接出示已下載的 QR Code 入場，不必現場購票。進館後確認水豚活動狀態及當日海豚表演時間。',
        ['成人票 2 張已購買', 'QR Code 直接入場', '09:00 起可用', '確認當日活動表'],
        { type: 'walk', label: '館內步行', route: '入口 → 2F Aqua Jungle', duration: '約 20-30 分' },
        'Maxell Aqua Park Shinagawa', 'tokyo', '—', '已付款 NT$1,168 / 2 位成人；含 No-show Refund',
        'https://www.aqua-park.jp/aqua/info/'),
      mkStop('d5s4', '10:10 - 10:40', '水豚展示・餵食活動候補', '2F Aqua Jungle 水豚展示區',
        '先看水豚展示；官方目前標示「水豚餵食」因健康管理暫停。若 9/23 前恢復，於 9/16 00:00 起預約可選場次，並在預約時間 5 分鐘前報到；若未恢復則改為一般觀看與 Aqua Jungle 探索。',
        ['水豚展示', '9/16 起查預約', '每場限 8 人', '未恢復則一般觀看'],
        { type: 'walk', label: '館內', route: 'Aqua Jungle 水豚展示區', duration: '約 30 分' },
        'Maxell Aqua Park Shinagawa Capybara', 'tokyo', '—', '餵食 ¥400 / 人；3 歲以下不可參加',
        'https://www.aqua-park.jp/aqua/event/31695/index.html',
        [
          { icon: 'clock', text: '若活動恢復，使用日前 7 天 00:00 開放預約；9/23 的預約起點是 9/16 00:00（日本時間）。' },
          { icon: 'child', text: '活動限 4 歲以上；3 歲以下孩子可同行進展示室，但基於安全不能餵食。' },
        ]),
      mkStop('d5s5', '10:40 - 12:30', '海豚表演與館內親子探索', 'The Stadium・Wonder Tube・Jellyfish Ramble',
        '依入館後確認的時刻選一場海豚表演，表演前 15-20 分鐘前往 The Stadium。其餘時間走訪 Wonder Tube、企鵝／海豹區與水母區；若水豚餵食恢復，優先以已預約時段調整這段順序。',
        ['海豚表演', '提前 15-20 分入場', 'Wonder Tube', '水母與企鵝'],
        { type: 'walk', label: '館內步行', route: 'Aqua Jungle → Wonder Tube → The Stadium', duration: '約 110 分' },
        'Maxell Aqua Park Shinagawa The Stadium', 'tokyo', '—', '已含入館費',
        'https://www.aqua-park.jp/aqua/event/'),
      mkStop('d5s6', '12:30 - 13:30', '品川王子大飯店周邊午餐', '品川王子大飯店／品川站高輪口',
        '離館後在品川王子大飯店餐廳或 Food Court 品川 Kitchen 用午餐，優先選快速出餐與兒童友善座位；13:30 左右回到品川站，保留國定假日候車緩衝。',
        ['親子午餐', '快速出餐', '高輪口周邊', '13:30 左右進站'],
        { type: 'walk', label: '步行', route: 'Aqua Park → 品川王子大飯店餐飲區', duration: '3-8 分' },
        'Shinagawa Prince Hotel Food Court Shinagawa Kitchen', 'tokyo', '午餐：飯店餐廳或 Food Court', '餐費約 ¥1,000~¥2,500 / 人',
        'https://www.princehotels.co.jp/shinagawa/restaurant/'),
      mkStop('d5s7', '13:30 - 14:20', 'Aqua Park 前往錦糸町', '品川 ➔ 錦糸町（JR 橫須賀・總武快速線直達）',
        '由高輪口步行回品川站，搭 JR 橫須賀線／總武快速線直達錦糸町，不需在東京站轉乘；下車後由北口步行約 3 分鐘至飯店。含步行與假日候車緩衝，目標 15:00 後辦理入住。',
        ['品川站高輪口', 'JR 直達', '不需轉乘', '15:00 後入住'],
        { type: 'train', label: 'JR + 步行', route: 'Aqua Park → 品川 → 錦糸町', duration: '約 40-50 分' },
        'Lotte City Hotel Kinshicho', 'kinshicho', '—', 'IC 卡車資依當日票價'),
      mkStop('d5s8', '15:00 - 16:30', '入住錦糸町樂天城市酒店 • 小孩午睡充電', '錦糸町樂天城市酒店 (Lotte City Hotel Kinshicho)',
        '15:00 後辦理 Check-in，先向櫃檯確認 Airporter 行李預定於 20:00 領取的安排；不把入住時間當作已送達時間。孩子可先在套房午睡，大人同步整理隨身用品與晚間採買清單。',
        ['15:00 後 Check-in', '行李 20:00 預定領取', '親子午休', '整理採買清單'],
      ...
      mkStop('d5s10', '18:30 - 20:30', '錦糸町晚餐 & 整理戰利品', 'OIOI／Arcakit 美食街 ➔ 飯店',
        '在錦糸町 OIOI 或 Arcakit 美食街享用親子友善晚餐，20:00 左右到樂天城市飯店櫃檯領取 Airporter 行李，再回房整理戰利品與打包；若尚未送達，以飯店與配送業者的即時通知為準。',
        ['親子友善晚餐', '20:00 領取 3 件行李', '回房整理戰利品', '隔日專車確認'],
        { type: 'walk', label: '步行', route: '錦糸町商圈 → 飯店', duration: '3-8 分' },
        'OIOI Kinshicho', 'kinshicho', '晚餐：日式定食／美食街', '餐飲預算約 ¥2,000~¥4,000 / 人',
        'https://www.0101.co.jp/054/info/'),
    ],
  },
  {
    id: 'd6',
    ...
    stops: [
      ...
      mkStop('d6s4', '12:30 - 14:25', '皇璽桂冠艙櫃檯與貴賓室禮遇', '成田機場 T1 長榮航空商務艙櫃檯 / 星空聯盟貴賓室',
        ...
        '成田空港 第1ターミナル 国際線 出発', 'tokyo', '午餐：貴賓室精緻美饌', '已含於機票費',
        'https://www.evaair.com/zh-tw/fly-prepare/at-the-airport/lounges/'),
